...
Adaptive-ODIL run:
    python adaptive_velocity_tracer_odil.py --adaptive 1 --Nx 64 --Nt 64

python3 adaptive_velocity_tracer_odil.py --adaptive 0 --outdir out_vel_fixed
python3 adaptive_velocity_tracer_odil.py --adaptive 1 --outdir out_vel_adaptive
"""
...
def operator_advection(ctx):
    ...
    finit = mod.where(it == 0, (u - extra.u_init[None, :, :]) / dx, zero)
    fu = mod.where(it == 0, zero, fu)
    ...
    res = [
        ("fu", fu * get_weight(ctx, "pde")),
        ("init", finit * get_weight(ctx, "init")),
        ("fimp", fimp * get_weight(ctx, "data")),
    ]

    # Spatial smoothness: kxreg * Laplacian(v).
    if args.kxreg:
        res += [
            ("vx_lap", laplace(vx_st) * get_weight(ctx, "smooth")),
            ("vy_lap", laplace(vy_st) * get_weight(ctx, "smooth")),
        ]

    # Time stationarity: ktreg * v_t.
    if args.ktreg:
        k = get_weight(ctx, "time") / dt
...
def make_problem(args):
    ...
    problem = odil.Problem(operator_advection, domain, extra, tracers=tracers)
