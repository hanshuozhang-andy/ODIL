...
This version:
  ...
  - adds an Adaptive-ODIL mode that balances PDE and initial-condition
    residual weights during training.
...
python3 adaptive_wave_odil.py --adaptive 0 --outdir out_wave_fixed
python3 adaptive_wave_odil.py --adaptive 1 --outdir out_wave_adaptive

Notes:
    - JAX supports the gradient-based optimizers used here, including L-BFGS-B.
    - Newton requires TensorFlow in the installed ODIL package:
          ODIL_BACKEND=tf python3 adaptive_wave_odil.py --optimizer newton ...
"""
...
def _group_norm_from_pinfo(pinfo, group_names):
...
def adaptive_epoch_func(problem, state, epoch, cbinfo):
    ...
    group_norms = {}
    for weight_name, residual_names in ADAPT_GROUPS.items():
        value = _group_norm_from_pinfo(cbinfo.pinfo, residual_names)
...
def operator_wave(ctx):
    ...
    fpde = mod.where(it == 0, ctx.cast(0), fu) * get_weight(ctx, "pde")
    finit = mod.where(it == 0, u - u0[None, :], ctx.cast(0)) * get_weight(ctx, "init")

    return [("pde", fpde), ("init", finit)]
...
def history_func(problem, state, epoch, hist, cbinfo):
    ...
    for name in args.adapt_weight_names:
        hist.append("weight_" + name, getattr(args, "w_" + name))
...
def make_problem(args):
    ...
    tracers = init_weight_tracers(args, {"pde": 1.0, "init": args.kimp})
    problem = odil.Problem(operator_wave, domain, extra, tracers=tracers)
...
def parse_args():
    ...
    parser.add_argument("--Nt", type=int, default=25, help="Grid size in t")
    parser.add_argument("--Nx", type=int, default=25, help="Grid size in x")
    parser.add_argument(
        "--kimp",
        type=float,
        default=1.0,
    ...
    # Defaults follow the existing working JAX run in Wave/out_wave_lbfgsb_jax.
    ...
    parser.set_defaults(every_factor=10)
