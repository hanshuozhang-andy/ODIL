#!/usr/bin/env python3
"""
Adaptive loss-weighting version of shape/body_shape_odil_jax.py.

Fixed-weight ODIL:
python Adaptive_ODIL/adaptive_body_shape_odil.py \
  --case all \
  --adaptive 0 \
  --outdir out_shape_fixed

Adaptive-weight ODIL:
python Adaptive_ODIL/adaptive_body_shape_odil.py \
  --case all \
  --adaptive 1 \
  --outdir out_shape_adaptive
"""
...
try:
    ...
    raise SystemExit(
        "adaptive_body_shape_odil.py requires JAX. Activate the same environment "
        "used for shape/body_shape_odil_jax.py, or install jax and jaxlib."
    ) from exc
...
ADAPTIVE_WEIGHT_KEYS = ("pde", "bc", "data", "eik", "area", "phi_anchor")
ADAPTIVE_UPDATE_KEYS = ("pde", "eik", "area")


def const_with_weights(const: Const, weights: Dict[str, float]) -> Const:
    return const._replace(
        w_pde=float(weights["pde"]),
        w_bc=float(weights["bc"]),
        w_data=float(weights["data"]),
        w_eik=float(weights["eik"]),
        w_area=float(weights["area"]),
        w_phi_anchor=float(weights["phi_anchor"]),
    )


def initial_loss_weights(const: Const) -> Dict[str, float]:
    return {
        "pde": float(const.w_pde),
        "bc": float(const.w_bc),
        "data": float(const.w_data),
        "eik": float(const.w_eik),
        "area": float(const.w_area),
        "phi_anchor": float(const.w_phi_anchor),
    }


def clipped_adaptive_loss(key: str, loss: float, args: argparse.Namespace) -> float:
    if not args.adapt_clip_losses:
        return loss
    lo = getattr(args, f"adapt_{key}_loss_min")
    hi = getattr(args, f"adapt_{key}_loss_max")
    return float(np.clip(loss, lo, hi))


def adaptive_weight_bounds(key: str, args: argparse.Namespace) -> Tuple[float, float]:
    lo = getattr(args, f"adapt_{key}_weight_min", args.adapt_min)
    hi = getattr(args, f"adapt_{key}_weight_max", args.adapt_max)
    return float(lo), float(hi)


def adaptive_norms(aux: Dict[str, Array], weights: Dict[str, float], kphi: float, args: argparse.Namespace) -> Dict[str, float]:
    norms: Dict[str, float] = {}
    for key in ADAPTIVE_UPDATE_KEYS:
        if weights[key] <= 0.0:
            continue
        if key not in aux:
            continue
        loss = float(jnp.maximum(aux[key], 0.0))
        if key == "eik":
            loss *= float(kphi) ** 2
        loss = clipped_adaptive_loss(key, loss, args)
        value = float(np.sqrt(max(loss, 0.0)))
        if value > args.adapt_eps:
            norms[key] = value
    return norms
...
def update_adaptive_weights(
    ...
    norms = adaptive_norms(aux, weights, kphi, args)
    if len(norms) < 2:
        return weights
    target = float(np.exp(np.mean(np.log(list(norms.values())))))
    new_weights = dict(weights)
    updates = []
    for key, norm in norms.items():
        old = float(weights[key])
        new = old * (target / norm) ** args.adapt_alpha
        wmin, wmax = adaptive_weight_bounds(key, args)
        new = float(np.clip(new, wmin, wmax))
        new_weights[key] = new
        updates.append(f"{key}:{old:.4g}->{new:.4g}")
    print("adaptive weights: " + ", ".join(updates), flush=True)
    return new_weights
...
def train(
    ...
    kphi0: float = 10.0,
    kphi_min: float = 1.0,
    adaptive_args: argparse.Namespace | None = None,
) -> PyTree:
    m = tree_zeros_like(params)
    vv = tree_zeros_like(params)
    weights = initial_loss_weights(const)
    hist_keys = [
        "epoch", "loss", "pde", "cont", "mom", "bc", "data", "eik", "area", "phi_anchor",
        "err_u", "err_chi", "kphi",
        "weight_pde", "weight_bc", "weight_data", "weight_eik", "weight_area", "weight_phi_anchor",
    ]
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for epoch in range(1, epochs + 1):
            ...
            const_epoch = const_with_weights(const, weights)
            if mode == "forward":
                (val, aux), grads = value_grad_forward(params, const_epoch)
            else:
                (val, aux), grads = value_grad_inverse(params, const_epoch, float(kphi))
            params, m, vv = adam_step(params, grads, m, vv, epoch, lr)
            do_adapt = (
                adaptive_args is not None
                and mode == "inverse"
                and adaptive_args.adaptive
                and adaptive_args.adapt_every
                and epoch % adaptive_args.adapt_every == 0
            )
            if do_adapt:
                weights = update_adaptive_weights(weights, aux, float(kphi), adaptive_args)
            if epoch == 1 or epoch % report_every == 0 or epoch == epochs:
                ...
                row = {
                    ...
                    "kphi": kphi,
                    **aux_host,
                    **{"weight_" + key: weights[key] for key in ADAPTIVE_WEIGHT_KEYS},
                }
                writer.writerow(row)
                f.flush()
                print(
                    ...
                    f"wpde={weights['pde']:.3g} wbc={weights['bc']:.3g} weik={weights['eik']:.3g}",
                    flush=True,
                )
    return params
...
def adaptive_loss_history(key: str, hist: Dict[str, np.ndarray]) -> np.ndarray:
    vals = np.asarray(hist[key], dtype=float)
    if key == "eik":
        vals = vals * np.asarray(hist.get("kphi", 1.0), dtype=float) ** 2
    return vals


def save_adaptive_history(case: str, outdir: str, hist_path: str, args: argparse.Namespace) -> None:
    ...
    loss_labels = {
        "pde": "PDE loss",
        "eik": "effective eikonal loss (kphi^2 * eik)",
        "area": "area loss",
    }
    for ax, key in zip(axes, ADAPTIVE_UPDATE_KEYS):
        raw = adaptive_loss_history(key, hist)
        lo = getattr(args, f"adapt_{key}_loss_min")
        hi = getattr(args, f"adapt_{key}_loss_max")
        clipped = np.clip(raw, lo, hi) if args.adapt_clip_losses else raw
        weight = hist.get(f"weight_{key}")

        ax.axhspan(lo, hi, color="#1f77b4", alpha=0.08, label="loss clip range")
        ax.semilogy(epoch, np.maximum(raw, 1e-16), color="0.25", lw=1.25, label="raw/effective loss")
        ax.semilogy(epoch, np.maximum(clipped, 1e-16), color="#1f77b4", lw=1.8, label="loss used for update")
        ax.set_ylabel(loss_labels[key])
        ax.grid(True, which="both", alpha=0.22, linewidth=0.7)

        if weight is not None:
            axw = ax.twinx()
            axw.plot(epoch, weight, color="#d62728", lw=1.7, label=f"weight_{key}")
            axw.set_ylabel(f"w_{key}", color="#d62728")
            axw.tick_params(axis="y", labelcolor="#d62728")
            wlo, whi = adaptive_weight_bounds(key, args)
            axw.set_ylim(wlo - 0.08 * (whi - wlo), whi + 0.08 * (whi - wlo))
        ...
        if weight is not None:
            ...
            handles += h2
            labels += l2
        ax.legend(handles, labels, ncol=3, frameon=False, loc="best", handlelength=1.5, columnspacing=0.9)

    axes[-1].set_xlabel("epoch")
    fig.suptitle(f"{case.capitalize()} bounded adaptive loss weighting", fontsize=13)
    path = os.path.join(outdir, f"adaptive_history_{case}.png")
...
def run_case(args: argparse.Namespace, case: str) -> None:
    ...
    if args.viz_only:
        ...
        save_adaptive_history(case, outdir, inv_hist, args)
        return
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--adaptive", type=int, default=0, help="Enable adaptive inverse loss weighting")
    p.add_argument("--adapt_every", type=int, default=200, help="Epoch interval for adaptive weight updates")
    p.add_argument("--adapt_alpha", type=float, default=0.5, help="Adaptive weight update exponent")
    p.add_argument("--adapt_eps", type=float, default=1e-12, help="Small number for safe adaptive updates")
    p.add_argument("--adapt_min", type=float, default=1e-5, help="Minimum adaptive weight")
    p.add_argument("--adapt_max", type=float, default=1e5, help="Maximum adaptive weight")
    p.add_argument("--adapt_clip_losses", type=int, default=1, help="Clip pde/eik/area losses before adaptive updates")
    p.add_argument("--adapt_pde_loss_min", type=float, default=1e-3, help="Minimum PDE loss used for adaptive updates")
    p.add_argument("--adapt_pde_loss_max", type=float, default=1.0, help="Maximum PDE loss used for adaptive updates")
    p.add_argument("--adapt_eik_loss_min", type=float, default=1e-4, help="Minimum effective kphi^2*eik loss used for adaptive updates")
    p.add_argument("--adapt_eik_loss_max", type=float, default=1e-2, help="Maximum effective kphi^2*eik loss used for adaptive updates")
    p.add_argument("--adapt_area_loss_min", type=float, default=1e-4, help="Minimum area loss used for adaptive updates")
    p.add_argument("--adapt_area_loss_max", type=float, default=4e-3, help="Maximum area loss used for adaptive updates")
    p.add_argument("--adapt_pde_weight_min", type=float, default=0.5, help="Minimum adaptive PDE weight")
    p.add_argument("--adapt_pde_weight_max", type=float, default=2.0, help="Maximum adaptive PDE weight")
    p.add_argument("--adapt_eik_weight_min", type=float, default=2e-3, help="Minimum adaptive eikonal weight")
    p.add_argument("--adapt_eik_weight_max", type=float, default=5e-2, help="Maximum adaptive eikonal weight")
    p.add_argument("--adapt_area_weight_min", type=float, default=1e-3, help="Minimum adaptive area weight")
    p.add_argument("--adapt_area_weight_max", type=float, default=5e-3, help="Maximum adaptive area weight")
