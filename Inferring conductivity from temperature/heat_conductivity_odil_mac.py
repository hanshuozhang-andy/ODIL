#!/usr/bin/env python3
"""
Standalone PyTorch reproduction of the ODIL example
"Inferring conductivity from temperature" from pgae005.pdf.

Run on macOS:
    python3 -m pip install numpy scipy matplotlib torch
    python3 heat_conductivity_odil_mac.py --epochs 3000

Fast test:
    python3 heat_conductivity_odil_mac.py --epochs 1000 --Nx 48 --Nt 48 --ref_Nx 128 --ref_Nt 128

Closest to the official source initialization:
    python3 heat_conductivity_odil_mac.py --zero_init --epochs 8000

Outputs:
    out_heat_mac/heat_results.png
    out_heat_mac/history.png
    out_heat_mac/model.pt
"""
...
def plot_results(outdir, t, x, ref_u, u_pred, mask, us, k_ref, k_pred):
    ...
    axes[2].set_title("error + data points")
    axes[2].set_xlabel("x")
...
def parse_args():
    ...
    p.add_argument("--noise", type=float, default=0.0, help="Gaussian noise added to data")
    ...
    p.add_argument("--outdir", type=str, default="out_heat_mac")
    return p.parse_args()
